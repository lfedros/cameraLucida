...
%% plot stimulus responses
    ...
    yval(1) = min([tuning.aveResp(:); tuning_cut.aveResp(:)]);
    yval(2) = max([tuning.aveResp(:); tuning_cut.aveResp(:)]);
...
xlabel(sprintf('%d deg (\\Delta pref dir)', d_dir_vals(iStim)));
...
%     print(fullfile(saveTo,[db.animal, '_', num2str(db.neuron_id), '_dendrotomy']) , '-painters', '-dpdf', '-fillpage');
    ...
    print(fullfile(saveTo,'responses') ,  '-dpng');
