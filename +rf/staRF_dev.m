function [spaceRF, RF, rfTimes, itsKernel] =  staRF_dev(resp, stimFrames, stimFrameTimes, tKernel)
...
RFtimesInFrames = numel(tKernel);
lagInFrames = numel(tKernel)-1;
...
spaceRF = pinv(tKernel)*RF';

spaceRF = reshape(spaceRF, xPx*yPx, nN);
RF = reshape(RF, xPx*yPx, nN, []);
...
[ipx, in] = ind2sub([yPx*xPx, nN], idx(1));

itsKernel = squeeze(makeVec(RF(ipx,in,:)));

spaceRF = reshape(spaceRF, yPx,xPx, nN);
RF = permute(reshape(RF, yPx,xPx, nN, []), [ 1 2 4 3]);
