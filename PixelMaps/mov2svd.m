...
exp_str = num2str(ops.expts(1));
for iE = 2:numel(ops.expts)
    exp_str = [exp_str,'_', num2str(ops.expt(iE))];

end

regFileLoc = fullfile(ops.RegFileBinLocation, ops.mouse_name, ops.date, exp_str);
regFile = sprintf('%s_%s_%s_plane%d.bin' ,  ops.mouse_name, ops.date, exp_str, ops.iplane);


ops.RegFile = fullfile(regFileLoc, regFile);
%% load the data 
