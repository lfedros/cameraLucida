...
for iDb = 1:nDb
spine_peak{iDb} = cat(2, neuron(iDb).spines(:).d_peak)';
...
ave_o(iDb) = nanmedian(spine_peak{iDb}(spine_type{iDb} == 'o'));
ave_p(iDb) = nanmedian(spine_peak{iDb}(spine_type{iDb} == 'p'));
...
xlabel('median spine size p')
ylabel('median spine size o')
