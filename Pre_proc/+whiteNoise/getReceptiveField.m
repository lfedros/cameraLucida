function [receptiveFields, explainedVariance, predictions, time] = ...
    ...
    stimFrames, stimTimes, RFtimesInFrames, lambdas, crossFolds)

%GETRECEPTIVEFIELD Returns spatiotemporal receptive field.
%   [receptiveField, explainedVariance, predictions, time] = ...
%    GETRECEPTIVEFIELD(traces, traceTimes, ...
%    stimFrames, stimTimes, RFtimesInFrames, lambdas, crossFolds) 
%    calculates the linear RF of the neuron.
%
...
%                       cross-validation
%   explainedVariance   [neuron x crossFold x lambda], each entry:
%                       explained variance for fitted RF for
%                       each neuron, cross val. fold, and lambda
...
lamMatrix_stim = makeLambdaMatrix([size(stimFrames,2), size(stimFrames,3), ...
...
predictions = NaN(nPerFold, crossFolds, size(traces,2), length(lambdas));

residuals = zTraces;

% get variances explained by stimulus
...
for fold = 1:crossFolds
    ...
    for lam = 1:length(lambdas)
        lms = lamMatrix_stim .* lambdas(lam);
        A = gpuArray([x_train; lms]);
        
        B = gather(A \ y_train);
        pred = x_test * B; % get prediction
        predictions(1:sum(~j), fold, valid, lam) = pred;
        explainedVariance(valid, fold, lam) = 1 - ...
            sum((y_test - pred) .^ 2,1) ./ ...
...
[~, bestStimLams] = max(mean(explainedVariance, 2), [], 3);
fprintf('  Optimal lambdas (of %d) to get RFs: ', length(lambdas))
for lam = 1:length(lambdas)
    fprintf('%d ', lam)
    ind = bestStimLams == lam & valid;
    A = [stim2; lamMatrix_stim .* lambdas(lam)];
    tr = padarray(residuals(:,ind), size(lamMatrix_stim,1), 'post');
    
    B = gather(gpuArray(A) \ gpuArray(tr));
    receptiveFields(:,ind) = B; % get RF kernel
    predictions(:,:,ind,1) = predictions(:,:,ind,lam);
end
fprintf('\n')
